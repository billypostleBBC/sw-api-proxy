...
function toLowercaseSet(input, transform = (value) => value) {
    return new Set(input
        .split(",")
        .map((value) => transform(value.trim().toLowerCase()))
        .filter(Boolean));
}
function baseEnv(parsed) {
    return {
        port: parsed.PORT,
...
export function loadProxyEnv(raw = process.env) {
...
export function loadRelayEnv(raw = process.env) {
    ...
    return {
        ...
        relayEmailDomainAllowlist: toLowercaseSet(parsed.RELAY_EMAIL_DOMAIN_ALLOWLIST, (value) => value.startsWith("@") ? value.slice(1) : value),
