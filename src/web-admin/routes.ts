...
function renderErrorBanner(errorKey?: string): string {
  if (!errorKey) {
    return "";
  }
...
function loginPageHtml(errorKey?: string): string {
  const body = `
...
function dashboardPageHtml(email: string, errorKey?: string): string {
  const body = `
...
export function registerWebAdminRoutes(app: FastifyInstance, deps: { authService: AuthService }): void {
